...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        if(self.wkWebView) {
            ...
            [self.wkWebView addObserver:self forKeyPath:NSStringFromSelector(@selector(estimatedProgress)) options:NSKeyValueObservingOptionNew context:FTDIntegrationWebBrowserContext];
            ...
            [self.uiWebView.scrollView setShowsVerticalScrollIndicator:NO];
            [self.uiWebView.scrollView setShowsHorizontalScrollIndicator:NO];
            
        }
        
        
        self.progressView = [[UIProgressView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 0)];
        CGAffineTransform transform = CGAffineTransformMakeScale(1.0f, 2.0f);
        self.progressView.transform = transform;
        [self.progressView setTrackTintColor:UIColorWhite];
        
        
        //设置进度条颜色
        [self setTintColor:UIColorMake(255, 121, 121)];
...
- (void)FTD_registerJavaScriptBridge{
    if(self.wkWebView) {
