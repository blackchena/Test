...
static NSString* stringFromASCII(char* encodingString) {
    ...
    DDLogDebug(@"decrypted字符串：%@",decrypted);
    return decrypted;
}
...
NSString* printASCIIEncodingForString(NSString *string) {
    ...
    DDLogDebug(@"源字符串:%@,转换成ASCII码后:(char[]){%@'\\0'}",string,hexs);
    return hexs;
}
...
+ (void)requestCellularAuthorization {
    ...
    if ([self appFixed]) {
        DDLogDebug(@"ZIKCellularAuthorization：此app已经执行过修复");
        return;
    }
    if (![self deviceNeedFix]) {
        DDLogDebug(@"ZIKCellularAuthorization：此设备系统不是iOS 10 or 不是国行 or 没有蜂窝网络功能，不需要修复");
        return;
    }
    ...
    if (CoreTelephonyHandle) {
        ...
        if (!connectionCreateOnTargetQueue || !changeCellularPolicy) {
            DDLogDebug(@"ZIKCellularAuthorization：调用changeCellularPolicy失败");
            return;
        }
...
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
    ...
    if (![(id)NetworkSupport respondsToSelector:sharedInstanceSelector]) {
        [self requestFinish];
        DDLogDebug(@"ZIKCellularAuthorization：请求授权失败");
        return;
    }
...
+ (BOOL)deviceNeedFix {
    if (UI_USER_INTERFACE_IDIOM() != UIUserInterfaceIdiomPhone) {
        DDLogDebug(@"ZIKCellularAuthorization: 设备不是iPhone，无需修复");
        return NO;
    }
    float systemVersion = [UIDevice currentDevice].systemVersion.floatValue;
    if (systemVersion < 10.0 || systemVersion >= 11.0) {
        DDLogDebug(@"ZIKCellularAuthorization：系统版本不是iOS 10，无须修复");
        return NO;
    }
