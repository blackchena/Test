...
- (void)setupConstraints
{
    ...
    if (_customView) {
        ...
        CGFloat widthBtn = roundf((width - 120)/2.0);
        ...
        NSDictionary *metrics = @{@"padding": @(padding),@"paddingB":@(widthBtn)};
        ...
        if ([self canShowButton]) {
            ...
            [self.contentView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-(paddingB@750)-[button(>=0)]-(paddingB@750)-|"
                                                                                     options:0 metrics:metrics views:views]];
        }
        // or removes from its superview
        else {
